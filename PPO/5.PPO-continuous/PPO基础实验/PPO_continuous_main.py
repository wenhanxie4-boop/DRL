...
from ENV.env_uav_0 import EnvCore
...
def main(args):
    ...
    run_name = f"{timestamp}_MLP_PPO"
    writer = SummaryWriter(log_dir=os.path.join("runs", run_name))

    save_dir = os.path.join(".", "results", "MLP_PPO")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "MLP PPO baseline for AoI-aware UAV data collection"
    )
    parser.add_argument(
        "--users_path",
        type=str,
    ...
    parser.add_argument("--use_tanh", type=str_to_bool, default=True)
