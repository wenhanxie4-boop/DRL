...
class EnvCore(gym.Env):
    """
    AoI-aware hierarchical UAV data collection environment.

    The high-level controller is an interpretable scheduler that selects one
    target user every option_interval steps according to AoI, remaining data,
    overflow risk, and distance. The low-level policy is still the ordinary
    continuous PPO actor and only controls dx, dy toward the selected target.
    """
    ...
    def __init__(
        ...
        # Low-level state:
        # UAV xy(2) + selected target(6) + NFZ(3) + high-level phase(1).
        self.obs_dim = 12
    ...
    def _select_high_level_target(self, force=False):
        current = next((u for u in self.Users if u.user_id == self.target_user_id), None)
        current_valid = current is not None and current.is_active and self._remaining_data(current) > 0

        if not force and current_valid and self.steps_since_target_switch < self.option_interval:
            return
        ...
        if not candidates:
            ...
            self.prev_target_dist = 0.0
            self.steps_since_target_switch = 0
            self.selected_target_history.append(-1)
            return

        best_user = max(candidates, key=self._user_priority_score)
        old_target_id = self.target_user_id
        self.target_user_id = best_user.user_id
        self.prev_target_dist = np.linalg.norm(self.uav.position[:2] - best_user.position[:2])
        self.steps_since_target_switch = 0
        self.selected_target_history.append(self.target_user_id)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.target_switch_count = 0
        self.selected_target_history = []
        self._select_high_level_target(force=True)
    ...
    def step(self, action):
        ...
        for user in self.Users:
            if user.user_id in served_user_ids:
                user.aoi = 0.0
            else:
                user.aoi += 1.0
        ...
        self._update_user_active_flags()
        self._select_high_level_target(force=False)
