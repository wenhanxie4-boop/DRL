...
if __name__ == '__main__':
    ...
    parser.add_argument("--adaptive_alpha", type=bool, default=True,
                        help="Whether to automatically learn the temperature alpha")
    ...
    base_save_dir = os.path.join('.', 'results', 'SAC')
    ...
    # 将 args 传给 SAC 以使用自定义超参数
    agent = SAC(state_dim, action_dim, max_action, args)
    replay_buffer = ReplayBuffer(state_dim, action_dim)

    writer = SummaryWriter(log_dir=f'runs/SAC_{timestamp}')

    # ================= 3. 训练主循环 =================
    max_train_steps = args.max_train_steps  # 使用传入的参数
    evaluate_freq = args.evaluate_freq  # 使用传入的参数
    random_steps = 25e3
    evaluate_num = 0
    evaluate_rewards = []
    total_steps = 0
    ...
    while total_steps < max_train_steps:
        ...
        if ep_total_reward > best_episode_reward and total_steps >= random_steps:
            ...
            print(
                f"🌟 [New Record] Episode {total_episodes}: Reward={best_episode_reward:.2f} | Envs & Model Saved to ./results/SAC/")
